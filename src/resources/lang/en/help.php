...
return [
    ...
    'whats_new_v3_list' => '<p><strong>Headline features:</strong></p>
        <ul>
            ...
            <li><strong>HR Manager Integration (16 PluginBridge capabilities)</strong>: Existing 3 ratting capabilities + 4 contribution analytics (Summary / ByCategory / Entries / CorpOutflows) + 5 new analytics (Trend / ActivityGaps / NetPosition / LifetimeSummary / CharacterPercentile) + MM tax compliance + director attribution + corp-wide member summary + corp financial summary (v3.0.1). State for milestone events tracked in <code>corpwalletmanager_member_milestone_state</code> so each transition publishes exactly once.</li>
        ...
        <ul>
            <li><strong>Manager Core</strong>: When installed, CWM publishes the 7 wallet / member topics above to MC\'s cross-plugin EventBus and the 16 PluginBridge capabilities are exposed.</li>
            ...
            <li><strong>HR Manager</strong>: Consumes the 16 capabilities to assemble per-member assessments and subscribes to the 3 milestone events for the member profile timeline. CWM and MM are both optional from HR\'s perspective.</li>
        </ul>',
...
GROUP BY corporation_id, prediction_method;',
    ...
    'contributions_intro' => 'CWM v3.0.0 introduces a per-character contribution cache that classifies every corp wallet journal row into a per-member bucket. The cache powers the Top Contributors leaderboard, the Profit Attribution dashboard, the Alliance Tax reconciliation tab, and 16 PluginBridge capabilities exposed to HR Manager.',
    ...
    'integrations_mc_capabilities_list' => '<ul>
        ...
        <li><strong>Corp-wide summaries (2):</strong> <code>contribution.getCorpMemberSummary</code> (v3.0.0: one grouped query returns a per-member financial roll-up for every member with wallet activity: lifetime contribution, ratting income, tax paid, withdrawals, net position, active months, and best-effort MM compliance) / <code>wallet.getCorpSummary</code> (v3.0.1: corp live balance summed across divisions + income / expense / net over a window + a zero-filled per-month trend, the "is the corp making or losing ISK" view that complements the per-member surfaces).</li>
    </ul>',
