 ...
 *
 * Mirrors Structure Manager's resolver verbatim per the project role-picker
 * pattern — keep the two implementations in sync.
 */
class DiscordRoleResolver
{
    ...
    public static function listRoles(): array
    {
        ...
        foreach ($providers as $provider) {
            try {
                ...
                Log::warning('[Corp Wallet Manager] DiscordRoleResolver: failed listing roles from ' . $provider . ': ' . $e->getMessage());
                continue;
            }
    ...
    /**
     * Aggregate label for the UI banner ("SeAT Broadcast + SeAT Connector").
     */
    ...
    /**
     * Short human label for a single provider.
     */
    ...
    /**
     * Indexed-by-id lookup map of every known role. Built once per call;
     * the settings UI passes the same map to describeRoleMention() for
     * every webhook in the list so each row resolves in O(1) without
     * re-running the provider queries.
     *
     ...
     *
     * Accepts every shape WebhookService::formatRoleMention() accepts:
     *   "<@&123456789>"  role mention
     *   "<@123456789>"   user mention
     *   "<@!123456789>"  user mention (nickname form)
     *   "123456789"      bare numeric id
     *
     * Returns the numeric string, or null when nothing snowflake-like is
     * present.
     */
    ...
    /**
     * Describe a stored role-mention value for display in the settings
     * UI. Returns null for an empty value, otherwise:
     *   [
     *     'id'     => '1227720362255712326' | null,
     *     'raw'    => the trimmed original string,
     *     'kind'   => 'role' | 'user' | 'unknown',
     *     'name'   => 'Corp Member' | null,
     *     'color'  => '#2ecc71' | null,
     *     'source' => 'discord-roles-table' | null,
     *     'known'  => bool,
     *   ]
     *
     * The 'kind' field classifies the raw value the same way the
     * WebhookService formatter does: <@&ID> and a bare number are role
     * candidates; <@ID> / <@!ID> are user mentions; anything else is
     * malformed and would be dropped at delivery time.
     *
     * Pass the precomputed roleLookupMap() as $map when describing
     * multiple values (e.g. one per webhook in a list) so each call is
     * O(1) instead of re-running the provider queries.
     *
     * @param array<string,array>|null $map  precomputed lookup; built on demand if null
     */
