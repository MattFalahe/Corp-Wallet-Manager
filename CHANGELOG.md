...
## [3.0.2] - Planetary Tax (2026-08-07)
...
- **Member milestone state now saves.** `MemberMilestoneState` declared an array as its Eloquent primary key (the table's real key is the composite corporation + character). Eloquent only supports a single-column key, so every attempt to update an existing row threw `Cannot access offset of type array on array`, logged as "MemberMilestoneNotifier: per-member evaluation failed". Rows inserted on the first pass but could never be updated afterwards, which meant milestone and stall state stopped advancing (and, with Manager Core installed, a stalled member could be re-announced instead of firing once). Saves are now pinned to the full composite key.
- **Backtest no longer overflows on large ISK bias values.** `bias_7d` / `bias_30d` were `DECIMAL(10,4)` (a ceiling near one million) but bias is an ISK-scale prediction error, so any corporation running a multi-billion ISK bias failed with `SQLSTATE[22003] 1264 Out of range value for column 'bias_7d'`. A forward-only migration widens both bias columns and the two MAPE columns, which had the same exposure.
- **One corporation can no longer abort the whole backtest run.** The job re-threw database errors, so a single corp's failure stopped every remaining corp from being scored. Per-corporation failures are now recorded and skipped, the run continues, and the failing corporations are listed on the job's entry in the Job Status log.
